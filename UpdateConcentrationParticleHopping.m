function [a1, r1, j] = UpdateConcentrationParticleHopping(N) 
    ...
    %% Solve in the Domain
    H_a = 0.5*N.lambda_eps*(N.eps_a/(N.dx.^2));
    H_r = 0.5*N.lambda_eps*(N.eps_r/(N.dx.^2));
...
%   H_r = (N.typicalTime*N.typicalDiffusion*N.eps_r)/(N.typicalLength^2*N.dx^2);
    ...
    a1(1) = N.a0(1) ...
        + N.dt*H_a*(-N.a0(1)*(1-N.p0(2))*exp(-tildeC*tildeV*C_a*(N.V_a(1)-N.V_a(2))) ... 
                    + N.a0(2)*(1-N.p0(1))*exp(-tildeC*tildeV*C_a*(N.V_a(2)-N.V_a(1)))) ... 
        + N.dt*N.lambda_in/(2*N.dx)*N.alpha_a*((N.Lambda_som/N.Lambda_som_max)*(1-N.p0(1)));
    
%     OK
%     disp('N.dt*N.lambda_in/(2*N.dx)*N.alpha_a*((N.Lambda_som/N.Lambda_som_max)*(1-N.p0(1)))');
%     N.dt*N.lambda_in/(2*N.dx)*N.alpha_a*((N.Lambda_som/N.Lambda_som_max)*(1-N.p0(1)))
    
    r1(1) = N.r0(1) ...
        + N.dt*H_r*(-N.r0(1)*(1-N.p0(2))*exp(-tildeC*tildeV*C_r*(N.V_r(1)-N.V_r(2))) ...
...
%     disp('- N.dt*N.lambda_out/(2*N.dx)*(N.beta_r*N.r0(1)*(1 - N.Lambda_som/N.Lambda_som_max))');
%     - N.dt*N.lambda_out/(2*N.dx)*(N.beta_r*N.r0(1)*(1 - N.Lambda_som/N.Lambda_som_max))
    
    % Boundary Flow at x = 1 
    a1(end) = N.a0(end) ...
        + N.dt*H_a*(-N.a0(end)*(1-N.p0(end-1))*exp(-tildeC*tildeV*C_a*(N.V_a(end)-N.V_a(end-1))) ...
            + N.a0(end-1)*(1-N.p0(end))*exp(-tildeC*tildeV*C_a*(N.V_a(end-1)-N.V_a(end)))) ... 
        - N.dt*N.lambda_out/(2*N.dx)*(N.beta_a*N.a0(end)*(1 - N.Lambda_tip/N.Lambda_tip_max));
% OK
%      disp('- N.dt*N.lambda_out/(2*N.dx)*(N.beta_a*N.a0(end)*(1 - N.Lambda_tip/N.Lambda_tip_max))');
%      - N.dt*N.lambda_out/(2*N.dx)*(N.beta_a*N.a0(end)*(1 - N.Lambda_tip/N.Lambda_tip_max))
    
    r1(end) = N.r0(end) ...
        + N.dt*H_r*(-N.r0(end)*(1-N.p0(end-1))*exp(-tildeC*tildeV*C_r*(N.V_r(end)- N.V_r(end-1))) ... 
            + N.r0(end-1)*(1-N.p0(end))*exp(-tildeC*tildeV*C_r*(N.V_r(end-1)-N.V_r(end)))) ... 
        + N.dt*N.lambda_in/(2*N.dx)*(N.alpha_r*(N.Lambda_tip/N.Lambda_tip_max)*(1-N.p0(end)));
%     OK
%     disp('N.dt*N.lambda_in/(2*N.dx)*(N.alpha_r*(N.Lambda_tip/N.Lambda_tip_max)*(1-N.p0(end)))');
%     N.dt*N.lambda_in/(2*N.dx)*(N.alpha_r*(N.Lambda_tip/N.Lambda_tip_max)*(1-N.p0(end)))
