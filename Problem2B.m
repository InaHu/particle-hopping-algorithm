...
N1.Lambda_som = 9;                      % Concentration in the Soma
N2.Lambda_som = N1.Lambda_som;
N1.Lambda_tip = 0.25;                   % Concentration in the growth cone of N1
N2.Lambda_tip = 0.25;                   % Concentration in the growth cone of N2

N1.Lambda_som_max = 10;
N2.Lambda_som_max = N1.Lambda_som_max;
N1.Lambda_tip_max = 0.5;               
N2.Lambda_tip_max = 0.5;

% Initial influx- and outflux values in all neurites
no_noflux = 1;                          % Set to 0 to have no flux for each Neurite
N1.alpha_a = no_noflux*1.2;
N1.alpha_r = no_noflux*0.3;
N2.alpha_a = no_noflux*1.2;
N2.alpha_r = no_noflux*0.3;

N1.beta_a = no_noflux*2;
N1.beta_r = no_noflux*1;
N2.beta_a = no_noflux*2;
N2.beta_r = no_noflux*1;
...
plotlefttip.PlotBoxAspectRatio(2) = 2;
plotlefttip.Position(1) = 0.0;
...
l = legend('ANT','RET', '1-$\rho$');
xl = xlabel('Points on the domain $\Omega$');
...
plotsoma.PlotBoxAspectRatio(2) = 2;
...
xlabel('Points on the domain $\Omega$');
...
plotrighttip.PlotBoxAspectRatio(2) = 2;
...
saveas(gcf, ['Images/TimeEvolution2B',num2str(0)], 'pdf');
...
for i = 1:m
    ...
    if (mod(i,1000)==0)
        ...
        % Update plot
%         subplot(1,5,2),
%         title(['Neurite 1, Time = ' num2str(i*dt)]);
        h1(1).YData = N1a1;             % Plot updaten
        h1(2).YData = N1r1;
        h1(3).YData = 1-N1a1-N1r1;
        
%         subplot(1,5,4),
%         title(['Neurite 2, Time = ' num2str(i*dt)]);
        h2(1).YData = N2a1;             % Plot updaten
        h2(2).YData = N2r1;
        ...
        if (mod(i,30000)==0)
            saveas(gcf, ['Images/TimeEvolution2B',num2str(i*dt*10)], 'pdf');
        end
    end
    ...
    Development_MassWholeSystem(i) = (N1.Lambda_som + N1.Lambda_tip +  N2.Lambda_tip) ...
        + sum(N1r1(:))/n + sum(N1a1(:))/n + sum(N2r1(:))/n + sum(N2a1(:))/n; 
    
    % Calculate entropy
    eN1 = N1a1.*log(N1a1) + N1r1.*log(N1r1) + (1 - N1p1).*log(1 - N1p1) + N1a1.*N1.V_a + N1r1.*N1.V_r ;
    eN2 = N2a1.*log(N2a1) + N2r1.*log(N2r1) + (1 - N2p1).*log(1 - N2p1) + N2a1.*N2.V_a + N2r1.*N2.V_r ;
...
% set(gca,'FontSize',12,'FontWeight','bold');
% 
% saveas(gcf, ['Images/DevelopmentPoolConcentration2B',num2str(i*dt*10)], 'pdf');
...
% set(gca,'FontSize',12,'FontWeight','bold');
% 
% saveas(gcf, ['Images/DevelopmentMass2B',num2str(i*dt*10)], 'pdf');
...
% set(gca,'FontSize',12,'FontWeight','bold');
% 
% saveas(gcf, ['Images/DevelopmentEntropy2B',num2str(i*dt*10)], 'pdf');
